...
static void pressed_left(GtkGestureClick *gesture, int n_press, double x,
...
static void released_left(GtkGestureClick *gesture, int n_press, double x,
                          double y, GtkWidget *area) {
  drawing = false;
}

static void pressed_right(GtkGestureClick *gesture, int n_press, double x,
                          double y, GtkWidget *area) {
  prev_x = x;
  prev_y = y;
  erasing = true;
...
static void released_right(GtkGestureClick *gesture, int n_press, double x,
                           double y, GtkWidget *area) {
  erasing = false;
  for (auto window : windows)
...
static void pressed_middle(GtkGestureClick *gesture, int n_press, double x,
                           double y, GtkWidget *area) {
  clear_surface(area);
  gtk_widget_queue_draw(area);
}

static void on_motion(GtkEventControllerMotion *self, double x, double y,
                      gpointer data) {
...
* { margin: 0; padding: 0; border: none; border-radius: 0; })"""");
    ...
    g_signal_connect_after(motion, "motion", G_CALLBACK(on_motion),
                           drawing_area);
    ...
    g_signal_connect_after(press, "pressed", G_CALLBACK(pressed_left),
                           drawing_area);

    GtkGesture *press2 = gtk_gesture_click_new();
    gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(press2),
                                  GDK_BUTTON_SECONDARY);
    gtk_widget_add_controller(GTK_WIDGET(window), GTK_EVENT_CONTROLLER(press2));
    g_signal_connect_after(press2, "pressed", G_CALLBACK(pressed_right),
                           drawing_area);

    GtkGesture *press3 = gtk_gesture_click_new();
    gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(press3),
                                  GDK_BUTTON_MIDDLE);
    gtk_widget_add_controller(GTK_WIDGET(window), GTK_EVENT_CONTROLLER(press3));
    g_signal_connect_after(press3, "pressed", G_CALLBACK(pressed_middle),
                           drawing_area);

    GtkGesture *release = gtk_gesture_click_new();
    gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(release),
                                  GDK_BUTTON_PRIMARY);
    gtk_widget_add_controller(GTK_WIDGET(window),
                              GTK_EVENT_CONTROLLER(release));
    g_signal_connect_after(release, "released", G_CALLBACK(released_left),
                           drawing_area);

    GtkGesture *release2 = gtk_gesture_click_new();
    gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(release2),
                                  GDK_BUTTON_SECONDARY);
    gtk_widget_add_controller(GTK_WIDGET(window),
                              GTK_EVENT_CONTROLLER(release2));
    g_signal_connect_after(release2, "released", G_CALLBACK(released_right),
                           drawing_area);
